...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item 设函数$f(x)=\mathrm{e}^{\frac{1}{x^{2}}}\arctan\frac{x-1}{x+1}$.指出$f
              (x)$的间断点，并判断其类型.
          \vspace{8em}

    \item 设$f(x)=x^{n}+a_{1} x^{n-1}+\cdots+a_{n-1}x+a_{n}$为$n$次实系数多项式.证明当$n$为奇数时，方程$f
              (x)=0$至少有一个实根.
          \vspace{8em}

    \item 设$f(x)$在$x=2$处可导，$f(2)\neq0$，求$\lim_{x\to\infty}\left[\frac{f\left(2+\frac{1}{x}\right)}{f(2)}
                  \right]^{x}$.
          \vspace{9em}

    \item 设一个雪球以$2\mathrm{cm}^{3}/\mathrm{min}$的速度融化.设雪球在融化过程中始终保持球形.求当雪球半径为$1
              0\mathrm{cm}$的时候半径变化的速率.
          \vspace{10em}

    \item 写出函数$f(x)=x\cos x$带Peano余项的五阶Maclaurin公式.
          \vspace{12em}
\end{enumerate}
...
\begin{enumerate}
    \setcounter{enumi}{15}
    \item 设数列$\{x_{n}\}$由递推公式$x_{1}=\sqrt{6}$，$x_{n+1}=\sqrt{6+x_{n}}$给出.证明$\lim
              _{n\to\infty}x_{n}$存在并求极限值.
          \vspace{12em}

    \item 设$0<a<b$，函数$f(x)$在闭区间$[a,b]$上连续，在开区间$(a,b)$内可导.证明存在$\xi
              \in(a,b)$使得
          \[
...
\begin{enumerate}
    ...
    \item \textbf{Solution}.
          \[
              \begin{aligned}
                  l= {} & \lim_{x\to0}\left(\mathrm{e}^{x}-\sin x\right)^{\frac{1}{x^2}}=\exp\left[{\lim\limits_{x\to0} \frac{1}{x^2}\ln\left(\mathrm{e}^x-\sin x\right)}\right]                 \\
                  = {}  & \exp\left({\lim\limits_{x\to0}\frac{\mathrm{e}^x-\cos x}{\mathrm{e}^x-\sin x}\cdot\frac{1}{2x}}\right)= \mathrm{e}^{\lim\limits_{x\to0}\frac{\mathrm{e}^x-\cos x}{2x}} \\
                  = {}  & \exp\left({\lim\limits_{x\to0}\frac{\mathrm{e}^x+\sin x}{2}}\right)=\sqrt{\mathrm{e}}.
              \end{aligned}
          \]

    \item \textbf{Solution}. 因为$\ln f(x)=\tan x\ln x$，

          所以
          \[
              f'(x)=f(x)\cdot\left(\tan x\ln x\right)'=x^{\tan x}\left(\sec^{2} x \ln
              x + \frac{\tan x}{x}\right),
          \]
    ...
    \item \textbf{Solution}. $f\left(0^{+}\right)=b$，$f\left(0^{-}\right
              )=1$，由连续性知$b=1$.

          求导得到$f'_{+}(0)=-2$，$f'_{-}(0)=a$，由导数存在知$a=-2$.
...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item \textbf{Solution}. 函数$f(x)$的间断点为$0,-1$. 因
          \[
    ...
    \item \textbf{Solution}.
          \[
              \begin{aligned}
                  \lim_{x\to\infty}\left[\frac{f\left(2+\frac{1}{x}\right)}{f(2)}\right]^{x} = {} & \exp\left[{\lim\limits_{x\to\infty}x\ln\frac{f\left(2+\frac{1}{x}\right)}{f(2)}}\right]                       \\
                  = {}                                                                            & \exp\left[{\lim\limits_{x\to\infty}x\ln\left(1+\frac{f\left(2+\frac{1}{x}\right)-f(2)}{f(2)}\right)}\right]   \\
                  = {}                                                                            & \exp\left[{\frac{1}{f(2)}\lim\limits_{x\to\infty}x\left[f\left(2+\frac{1}{x}\right)-f(2)\right]}\right]       \\
                  = {}                                                                            & \exp\left[{\frac{1}{f(2)}\lim\limits_{x\to\infty}\frac{f\left(2+\frac{1}{x}\right)-f(2)}{\frac{1}{x}}}\right] \\
                  = {}                                                                            & \exp\left[{\frac{f'(2)}{f(2)}}\right].
              \end{aligned}
          \]

    \item \textbf{Solution}. 根据几何关系得到
          \[
              V=\frac{4\pi r^{3}}{3}.
          \]
          两边对$t$求导得到
...
\begin{enumerate}
    \setcounter{enumi}{15}
    \item \textbf{Proof}. 显然$x_{2}>x_{1}$，设$x_{k+1}>x_{k}$，由$x_{k+2}^{2}-x_{k+1}^{2}=6+x_{k+1}-
    ...
    \item \textbf{Proof}. 显然函数$F(x)=\frac{f(x)}{x}$和$G(x)=\frac{1}{x}$在闭区间$[
                  a,b]$上连续，在开区间$(a,b) $内可导，

          并且$G(x)$的导数不为0.由Cauchy中值定理知存在$\xi\in(a,b)$使得
          \[
