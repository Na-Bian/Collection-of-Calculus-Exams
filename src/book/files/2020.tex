...
\begin{enumerate}
    ...
    \item 极限$\lim_{x\to\infty}\left[\frac{x^{2}}{(x-a)(x+b)}\right]^{x}$=（\qquad）.
    ...
    \item 设$F(x)=
              \begin{cases}
                  \frac{f(x)}{x}, & x\neq0, \\
                  f'(0),          & x=0
              \end{cases}$，其中$f'(0)=1$，$f(0)=0$，则$x=0$是$F(x)$的（\qquad）.
    ...
    \item 函数$f(x)$在$x=0$处可导，且$f(0)=0$，则$\lim_{x\to0}\frac{x^{2}f(x)-2f(x^{3})}{x^{3}}$=（\qquad）.
    ...
    \item 设$f(x)$有二阶连续导数，且$f'(0)=0$，$\lim_{x\to0}\frac{f''(x)}{|x|}=1$，则（\qquad）.
...
\begin{enumerate}
    ...
    \item \textbf{Solution}. 因为
          \[
              \frac{\mathrm{d}x}{\mathrm{d}y}=\frac{1}{\ln
                  x+1},\quad\frac{\mathrm{d}^{2} x}{\mathrm{d}y^{2}}=\frac{\mathrm{d}}{\mathrm{d}y}
          ...
          \]
          所以当$x=\mathrm{e}$时，$\frac{\mathrm{d}^{2} x}{\mathrm{d}y^{2}}=-\frac{1}{8\mathrm{e}}$.

    \item \textbf{Solution}. 因$f'(x)=3x^{2}-6x-9=3(x+1)(x-3)$，故$f(x)$在$[-2
                  ,2]$内的极值点为$x=-1$.
