...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item 设$x=\varphi(y)$是$f(x)=\ln x+\arctan x$的反函数，求$\varphi'\left
              (\frac{\pi}{4}\right)$.
          \vspace{10em}

    \item 设曲线$y=y(x)$由方程$\mathrm{e}^{xy}+\ln\frac{y}{x+1}=2$确定，求曲线在$x
              =0$处的切线方程.
          \vspace{10em}

    \item 设函数$f(x)$在$x=0$处可导，且$f(0)=0$，$f'(0)=2$，计算$\lim_{x\to0}
              \frac{x^{2}f(x)-2f(x^{3})}{x^{3}}$.
          \vspace{10em}

    \item 有一长度为$5\mathrm{m}$的梯子贴靠在铅直的墙上，假设其下端沿地板离开墙角而滑动.当梯子下端离开墙角$3
...
\begin{enumerate}
    \setcounter{enumi}{15}
    \item 设$x_{1}>0$，$x_{n+1}=3+\frac{4}{x_{n}}(n=1,2,\cdots)$，证明：$\lim
              _{n\to\infty}x_{n}$存在并求其值.
          \vspace{12em}

    \item 设函数$f(x)$在$[0,1]$上连续，$(0,1)$内可导，$c\in(0,1)$.证明：$\exists
              \,\xi,\eta\in[0,1]$，使得
          \[
...
\begin{enumerate}
    ...
    \item \textbf{Solution}. $l=\lim_{x\to0}\exp\left[{\frac{1}{\mathrm{e}^{x}-1}\ln\frac{\ln(1+x)}{x}}\right]
              =\exp\left[{\lim\limits_{x\to0}\frac{1}{\mathrm{e}^{x}-1}\ln\frac{\ln(1+x)}{x}}\right]$.
    ...
    \item \textbf{Solution}. $f(x)$的间断点为$x=k\pi$，$x=k\pi+\frac{\pi}{2}$，$k
              =0,\pm1,\pm2,\cdots$，理由如下：

          $x=0$是跳跃间断点.因为
          \[
              \lim_{x\to0^+}\exp\left({\frac{|x|}{\tan x}}\right)=\exp\left({\lim\limits_{x\to0^+}\frac{x}{\tan
                      x}}\right)=\mathrm{e},\quad \lim_{x\to0^-}\exp\left({\frac{|x|}{\tan x}}\right)=\exp\left({\lim_{x\to0^-}\frac{-x}{\tan x}}\right)=\mathrm{e}^{-1}.
          \]

          $x=k\pi(k=\pm1,\pm2,\cdots)$是无穷间断点.因为
          \[
              \lim_{x\to k\pi^+}\exp\left({\frac{|x|}{\tan x}}\right)=\mathrm{e}^{+\infty}=+\infty
              .
          \]

          $x=k\pi+\frac{\pi}{2}(k=0,\pm1,\pm2,\cdots)$是可去间断点.因为
          \[
              \lim_{x\to k\pi+\frac{\pi}{2}}\exp\left({\frac{|x|}{\tan x}}\right)=\mathrm{e}^{0}
              =1.
          \]
    ...
    \item \textbf{Solution}. 取$v(x)=x^{2}$，它的三阶以上的导数为0，
          ...
          所以
          \[
              y^{(n)}(0)=(-1)^{n-3}n(n-1)(n-3)!=\frac{(-1)^{n}n!}{n-2}(n>2).
          \]
          而$y'(0)=y''(0)=0$.

    \item \textbf{Solution}. 因为
          \[
              \frac{\mathrm{d}y}{\mathrm{d}x}=\frac{-x^{2}\sin x-2x\cos
...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item \textbf{Solution}. 当$x=1$时，$y=f(1)=\frac{\pi}{4}$.

          而
          \[
              f'(x)=\left(\ln x+\arctan x\right)'=\frac{1}{x}+\frac{1}{1+x^{2}},
          \]
          所以$f'(1)=1+\frac{1}{2}=\frac{3}{2}$，故
          \[
              \varphi'\left(\frac{\pi}{4}\right)=\frac{1}{f'(1)}=\frac{2}{3}.
          \]

    \item \textbf{Solution}. 原方程变为
          \[
              \mathrm{e}^{xy}(y+xy')+\frac{y'}{y}-\frac{1}{x+1}=0,
          \]
    ...
    \item \textbf{Solution}. $y'(x)=2f(x)f'(x)$，
    ...
    \item \textbf{Solution}. 设梯子上端离墙角距离为$s(m)$，下端离开墙角的距离为$x
              (m)$，有一长度为
          \[
              s=\sqrt{5^{2}-x^{2}}.
          \]
    ...
    \item \textbf{Solution}. 因$f'(0)=\lim_{x\to0}\frac{f(x)-f(0)}{x}=\lim_{x\to0}
              x\cos\frac{1}{x}=0$，
...
\begin{enumerate}
    ...
    \item \textbf{Proof}.
          ...
          \]
          知奇子列$\{x_{2k-1}\}$与偶子列$\{x_{2k}\}$均分别单调.

          由单调有界原理知奇子列$\{x_{2k-1}\}$与偶子列$\{x_{2k}\}$均收敛.
    ...
    \item \textbf{Proof}. 构造函数$F(x)=x^{2}f(1)+(c^{2}-1)f(x)$，则$F(x)$在$[
                  0,1]$上连续，$(0,1)$内可导.
