...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item 研究函数$f(x)=\lim_{x\to\infty}\frac{x+x^{2}\mathrm{e}^{nx}}{1+\mathrm{e}^{nx}}$的连续性.
          \vspace{10em}

    \item 设$y=g(x)$是$y=f(x)$的反函数，$f(x)$可导，$f(1)=2,f'(1)=-4$，求$y=
              g(1+x^{2})$在$x=1$处的导数.
          \vspace{10em}

    \item 设函数$f(x)$在$x=1$处二阶可导，且$f'(1)=0$，$f''(1)=0$，$y=f^{2}(x
    ...
    \item 将水以$4\mathrm{m}^{3}/\mathrm{min}$的速率注入一个圆锥形容器中，容器顶朝下倒立，它的高度为$8
...
\begin{enumerate}
    \setcounter{enumi}{15}
    \item 设$x_{n}>0$，$x_{n+1}+\frac{4}{x_{n}}<4(n=1,2,\cdots)$，证明：$\lim
              _{n\to\infty}x_{n}$存在并求其值.
          \vspace{12em}

    \item 设函数$f(x)$在$[0,b]$上具有二阶导数，且$|f''(x)|\leq M$，$f(x)$在$(
...
\begin{enumerate}
    ...
    \item \textbf{Solution}. 化简得$y(x)=\frac{1-\sqrt{1-x^{2}}}{x}$.
          ...
          所以
          \[
              y'(x)=\frac{-\frac{2x}{2\sqrt{1-x^{2}}}\cdot x-(1-\sqrt{1-x^{2}})}{x^{2}}
              =\frac{1-\sqrt{1-x^{2}}}{x^{2}\sqrt{1-x^2}}.
          \]

    \item \textbf{Solution}. 由题设有$x=0,y=1$，方程两边关于$x$求导得
          \[
              (1-x\mathrm{e}^y)y'=\mathrm{e}^y,\tag{$*$}
          \]
          \[
              y'(0)=\mathrm{e}.
          \]
    ...
    \item \textbf{Solution}. 因$\ln y=\sin x\ln(1+x^{2})$，所以
          ...
          即
          \[
              y'=\left(\cos x\ln(1+x^{2})+\frac{2x\sin x}{1+x^{2}}\right)y=(1+x^{2})^{\sin x}\left(\cos x\ln(1+x^{2})+\frac{2x\sin x}{1+x^{2}}\right),
          \]
          因此
          \[
              \mathrm{d}y=(1+x^{2})^{\sin x}\left(\cos x\ln(1+x^{2})+\frac{2x\sin x}{1+x^{2}}\right)\,\mathrm{d}x.
          \]
...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item \textbf{Solution}. 当$x>0$时，$\mathrm{e}^{nx}\to+\infty(n\to\infty
    ...
    \item \textbf{Solution}. 由题意得$g'(2)=\frac{1}{f'(1)}=-\frac{1}{4}$.
    ...
    \item \textbf{Solution}. 设时刻$t$容器中水的体积为$V\mathrm{m}^{3}$，水的高度为为$h
          ...
          当$h=5\mathrm{m}$时，$\frac{\mathrm{d}V}{\mathrm{d}t}=4\mathrm{m}^{3}/\mathrm{min}$时，$\frac{\mathrm{d}h}{\mathrm{d}t}
              =\frac{16}{25\pi}\mathrm{m}/\mathrm{min}$.即水面升高的速率是$\frac{16}{25\pi}\mathrm{m}/\mathrm{min}$.
\end{enumerate}

\section{证明题(每小题 5 分，共 10 分)}
\begin{enumerate}
    \setcounter{enumi}{15}
    \item \textbf{Proof}.由$x_{n+1}-x_{n}<4-\frac{4}{x_{n}}-x_{n}=-\left(\sqrt{x_{n}}
    ...
    \item \textbf{Proof}. 设$f(x)$在$x_{0}\in(0,b)$处取得最大值，则$f'(x_{0}
              )=0$.
