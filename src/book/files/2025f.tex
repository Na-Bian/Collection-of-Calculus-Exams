...
\begin{enumerate}
    ...
    \item 微分方程$y''-2y'+2y=\mathrm{e}^{x}\sin x+\mathrm{e}^{-x}$的特解形式应设为$y
          ...
          \begin{tasks}[label=\textbf{\Alph*.}, label-width=1.5em, column-sep=1em](2)
              \task $x\mathrm{e}^{x}(a\cos x+b\sin x)+c\mathrm{e}^{-x}$
              \task $\mathrm{e}^{x}(a\cos x+b\sin x)+c\mathrm{e}^{-x}$
              \task $\mathrm{e}^{x}(ax+b)\cos x+c\mathrm{e}^{-x}$
              \task $\mathrm{e}^{x}(a\cos x+b\sin x)+cx\mathrm{e}^{-x}$
          \end{tasks}

    \item 设函数$y=f(x)$在$(a,b)$内有二阶导数，点$(c,f(c))(a<c<b)$是曲线$y=f(x)$的拐点的一个充分条件为（\qquad）.

          \begin{tasks}[label=\textbf{\Alph*.}, label-width=1.5em, column-sep=1em](2)
              \task $f''(x)$在$(a,b)$内严格单调递增
              \task $f''(c)=0$
              \task $f''(x)$在$(a,b)$内严格单调递减
              \task $f''(c)=0$且$f''(x)$在$(a,b)$内严格单调递减
          \end{tasks}
    ...
    \item 若函数$f(x)$满足$\mathrm{d}f(x)=-\mathrm{e}^{\cos x}\sin x\,\mathrm{d}x$，且$f
          ...
          \begin{tasks}[label=\textbf{\Alph*.}, label-width=1.5em, column-sep=1em](2)
              \task $\mathrm{e}^{\cos x}-1$
              \task $\mathrm{e}^{\sin x}$
              \task $\mathrm{e}^{\sin x}-\mathrm{e}$
              \task $\mathrm{e}^{\cos x}-\mathrm{e}$
          \end{tasks}

    \item 下列反常积分发散的是（\qquad）.

          \begin{tasks}[label=\textbf{\Alph*.}, label-width=1.5em, column-sep=1em](2)
              \task $\int_{0}^{+\infty}\mathrm{e}^{-x}\,\mathrm{d}x$
              \task $\int_{-1}^{1}\frac{1}{\sqrt{1-x^{2}}}\,\mathrm{d}x$
              \task $\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}}\frac{1}{\tan x}\,\mathrm{d}x$
              \task $\int_{2}^{+\infty}\frac{1}{x\ln^{2} x}\,\mathrm{d}x$
          \end{tasks}
\end{enumerate}
...
\begin{enumerate}
    \setcounter{enumi}{6}
    \item $\int_{-1}^{1}\left(x\ln\left(x^{2026}+1\right)+\sqrt{1-x^{2}}\right
              )\,\mathrm{d}x=\underline{\hspace{3cm}}$.
          \vspace{1em}

    \item 曲线$y=\frac{1}{x}+\ln\left(1+\mathrm{e}^{x}\right)(x>0)$的斜渐近线为$\underline
              {\hspace{3cm}}$.
          \vspace{1em}
    ...
    \item 设函数$f(x)=\int_{0}^{1}|t-x|\mathrm{d}t(0<x<1)$，则$y=f(x),x=0,x=
...
\begin{enumerate}
    \setcounter{enumi}{10}
    \item 求极限$l=\lim_{x\to0}\frac{x^{2}(\cos x-1)}{\cos x-\mathrm{e}^{-\frac{x^2}{2}}}$.
          \vspace{10em}
    ...
    \item 若$f(x)=ax^{3}+bx^{2}+x$在$x=1$处取得极值$2$，求$f(x)$在区间$[-1,2
              ]$上的最大值和最小值.
          \vspace{10em}

    \item 设$f(x)$在$x=0$处可导，且$\lim_{x\to0}\frac{f(x)}{x}=1$，求极限$l=
              \lim_{x\to0}\frac{1}{x^{4}}\int_{0}^{x}tf(x^{2}-t^{2})\,\mathrm{d}t$.
          \vspace{10em}

    \item 求微分方程$(y^{2}-6x)\frac{\mathrm{d}y}{\mathrm{d}x}=-2y$的通解.
          \vspace{11em}

    \item 在$xOy$平面内，把连接点$O(0,0)$与点$P(1,0)$的线段$OP$剖分为$n$等分，各分点依次记为$P
...
\begin{enumerate}
    \setcounter{enumi}{16}
    \item 设$f(x)$可微，且满足$x=\int_{0}^{x}f(t)\,\mathrm{d}t+\int_{0}^{x}tf(
              t-x)\,\mathrm{d}t$，求$f(x)$.
          \vspace{12em}

    \item 已知曲线$L$的方程为$y=f(x)$，点$(3,2)$是它的一个拐点，直线$l_{1},l
...
\begin{enumerate}
    \item \textbf{Solution}. B.
          ...
          \[
              \begin{gathered}
                  f(1^{+})=\lim_{x\to1^+}\mathrm{e}^{-\frac{1}{x-1}}=0,\\
                  f(1^{-})=\lim_{x\to1^-}\mathrm{e}^{-\frac{1}{x-1}}=+\infty,
              \end{gathered}
          \]
    ...
    \item \textbf{Solution}. A.
          ...
          又
          \[
              \lim_{x\to0}\frac{f(x)+f'(x)}{x}=\lim_{x\to0}\frac{f(x)-f(0)}{x}+\lim_{x\to0}
              \frac{f'(x)-f'(0)}{x}=f'(0)+f''(0)=1,
          \]
          所以$f''(0)=1>0$. 故$f(0)$是$f(x)$的极小值.
...
\begin{enumerate}
    \setcounter{enumi}{6}
    \item \textbf{Solution}. $\frac{\pi}{2}$.

          因为函数$y=x\ln\left(x^{2026}+1\right)$是奇函数，故$\int_{-1}^{1}x\ln\left(x^{2026}
              +1\right)\,\mathrm{d}x=0$.

          由定积分的几何意义可得$\int_{-1}^{1}\sqrt{1-x^{2}}\,\mathrm{d}x=\frac{\pi}{2}$，所以
          \[
              \int_{-1}^{1}\left(x\ln\left(x^{2026}+1\right)+\sqrt{1-x^{2}}\right)\,\mathrm{d}x
              =\frac{\pi}{2}.
          \]

    \item \textbf{Solution}. $y=x$.
    ...
    \item \textbf{Solution}. $4$.
          ...
          因为$y'=\sqrt{\sin x}$，所以$\mathrm{d}s=\sqrt{1+y'^{2}}\,\mathrm{d}x=\sqrt{1+\sin
                  x}\,\mathrm{d}x=\left|\sin\frac{x}{2}+\cos\frac{x}{2}\right|\mathrm{d}x$，

          故
          \[
              s=\int_{0}^{\pi}\left|\sin\frac{x}{2}+\cos\frac{x}{2}\right|\mathrm{d}x=
              \int_{0}^{\pi}\left(\sin\frac{x}{2}+\cos\frac{x}{2}\right)\,\mathrm{d}x=4.
          \]

    \item \textbf{Solution}. $\frac{\pi}{3}$.

          因为$f(x)=\int_{0}^{x}(t-x)\,\mathrm{d}t+\int_{x}^{1}(x-t)\,\mathrm{d}t=x^{2}-x+\frac{1}{2}$.

          故旋转体的体积
          \[
              V=\int_{0}^{1}2\pi xf(x)\,\mathrm{d}x=2\pi\int_{0}^{1}x\left(x^{2}
...
\begin{enumerate}
    ...
    \item \textbf{Solution}. 当$x=9$时，$1+2t^{2}=9$，因为$t>1$，故$t=2$.
          ...
          \[\frac{\mathrm{d}y}{\mathrm{d}x}=\frac{\frac{\mathrm{d}y}{\mathrm{d}t}}{\frac{\mathrm{d}x}{\mathrm{d}t}}
              =\frac{\frac{\mathrm{e}^{1+2\ln t}}{1+2\ln t}\cdot\frac{2}{t}}{4t}=\frac{\mathrm{e}}{2(1+2\ln
                  t)},
          \]
          所以$\left.\frac{\mathrm{d}y}{\mathrm{d}x}\right|_{x=9}=\left.\frac{\mathrm{d}y}{\mathrm{d}x}
              \right|_{t=2}=\frac{\mathrm{e}}{2(1+2\ln 2)}$，且
          \[
    ...
    \item \textbf{Solution}. 由题可知$f(1)=2$，$f'(1)=0$，且$f'(x)=3ax^{2}+2
              bx+1$，
    ...
    \item \textbf{Solution}. 由$\lim_{x\to0}\frac{f(x)}{x}=1$可得$f(0)=0$，$f
              '(0)=1$.
    ...
    \item \textbf{Solution}. 方程变形为$\frac{\mathrm{d}x}{\mathrm{d}y}-\frac{3x}{y}=-\frac{y}{2}$.
    ...
    \item \textbf{Solution}. 点$P_{k}$的坐标为$\left(\frac{k}{n},0\right)$，过$Q
...
\begin{enumerate}
    ...
    \item \textbf{Solution}. 令$u=t-x$，则
          ...
          且$f''(x)=(f'(x))'=(-f(-x))'=f'(-x)=-f(x)$，即$f''(x)+f(x)=0$.

          此微分方程的特征方程为$r^{2}+1=0$，解得$r=\pm i$，所以可设$f(x)=A\cos x+B\sin x$.

          将$f(0)=1$，$f'(0)=-1$代入上式得$A=1$，$B=-1$，所以$f(x)=\cos x-\sin x$.

    \item \textbf{Solution}. 由题可知$f''(3)=0$，$f(3)=2$，$f(0)=0$，$l_{1}:
...
\begin{enumerate}
    \setcounter{enumi}{18}
    \item \textbf{Proof}. 令$F(t)=\int_{0}^{t}g(x)f'(x)\,\mathrm{d}x+\int_{0}^{1}
              f(x)g'(x)\,\mathrm{d}x-f(t)g(1)$，则
          \[
              F(1)=\int_{0}^{1}\left(g(x)f'(x)+f(x)g'(x)\right)\,\mathrm{d}x-f(1)g(1)=0,
          \]
          \[
              F'(t)=g(t)f'(t)-f'(t)g(1)=f'(t)\left[g(t)-g(1)\right].
          \]
          ...
          所以$F(a)\geq F(1)=0$，即
          \[
              \int_{0}^{a}g(x)f'(x)\,\mathrm{d}x+\int_{0}^{1}f(x)
              g'(x)\,\mathrm{d}x\geq f(a)g(1).
          \]

    \item \textbf{Proof}. 利用Taylor公式，$\forall\,x\in[-1,1]$，存在$\eta$介于$0$和$x$之间，使得
          \[
          ...
          所以
          \[
              3\int_{-1}^{1}f(x)\,\mathrm{d}x=3\int_{-1}^{1}\left(f'(0)x+\frac{f''(\eta)}{2}
