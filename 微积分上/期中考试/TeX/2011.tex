...
\begin{document}
  ...
  \begin{enumerate}
    \item[13.] 设$f(x)$在$[0,2]$上连续，在$(0,2)$内可微，且$f(0)\cdot f(2)>0$，$f
      (0)\cdot f(1)<0$，证明：存在$\xi\in(0,2)$，使$f'(\xi)=f(\xi)$.
      \vspace{9em}

    \item[14.] 设函数$y=f(x)$和它的反函数$x=\varphi(y)$均存在三阶导数，且$y'\neq0$，请推导出反函数的求导公式$\frac{\mathrm{d}x}{\mathrm{d}y}$，$\frac{\mathrm{d}^{2}
      x}{\mathrm{d}y^{2}}$，$\frac{\mathrm{d}^{3} x}{\mathrm{d}y^{3}}$【用$y'$，$y
      ''$，$y'''$表示】.
      \vspace{9em}

    \item[15.] 证明：当$x\geq1$时，有$\arctan x=\frac{\pi}{4}+\frac{1}{2}\arccos\frac{2x}{1+x^{2}}$.
      \vspace{10em}

    \item[16.] 设函数$f(x)$在$[a,+\infty)$上连续，在$(a,+\infty)$内可导，且$f'(x)
  ...
  \begin{enumerate}
    \item[17.] 分别叙述数列$x_{n}$有界和收敛（以$\lim_{n\to\infty}x_{n}=a$为例）的定义，并证明：收敛数列是有界数列.
      \vspace{12em}

    \item[18.] 如果记$\xi=\theta\cdot x$，$0<\theta<1$，则Lagrange中值公式$f(x)-f
  ...
  \begin{enumerate}
    ...
    \item \textbf{Solution}. 由题意可知$f(0)=y(0)=0$，$f'(0)=y'(0)=\left.\frac{2}{1+4x^{2}}
      \right|_{x=0}=2$，

      所以$\lim_{n\to\infty}nf\left(\frac{1}{n}\right)=\lim_{n\to\infty}\frac{f\left(\frac{1}{n}\right)-f(0)}{\frac{1}{n}-0}
      =f'(0)=2$.

    \item \textbf{Solution}. （1）因$\varphi(x)$在$x=1$处连续，所以$\varphi(1)=\lim
      _{n\to\infty}\varphi\left(\frac{n}{n+1}\right)=\lim_{n\to\infty}\sqrt[n]{n}
      =1$.

      （2）由导数定义，$f'(1)=\lim_{x\to1}\frac{f(x)-f(1)}{x-1}=\lim_{x\to1}\frac{(x-1)\varphi(x)}{x-1}
      =\lim_{x\to1}\varphi(x)=\varphi(1)=1$.

    \item \textbf{Solution}. $y'=\frac{1}{2\sqrt{x+\sqrt{x}}}\cdot\left(1+\frac{1}{2\sqrt{x}}
    ...
    \item \textbf{Solution}. $\frac{\mathrm{d}y}{\mathrm{d}x}=\frac{\frac{\mathrm{d}y}{\mathrm{d}t}}{\frac{\mathrm{d}x}{\mathrm{d}t}}
      ...
      t}{\tan t}$，$\left.\frac{\mathrm{d}^{2} y}{\mathrm{d}x^{2}}\right|_{t=\frac{\pi}{4}}
      =\frac{\sqrt{2}}{2}\left(1-\frac{\pi}{4}\right)$.

    \item \textbf{Solution}. $f(x)=\frac{1}{3}\left(\frac{1}{2x+1}+\frac{1}{x+2}\right
      )$，所以
      \[
    ...
    \item \textbf{Solution}. $f(x)$的间断点为$x=0$和$x=1$.
      ...
      =\lim_{x\to0}\frac{\arctan\frac{x+1}{x-1}}{\mathrm{e}^{\frac{1}{x^2}}}=0$，且$f
      (0)$无定义，

      所以$x=0$是$f(x)$的可去间断点；

      $\lim_{x\to1^-}f(x)=\lim_{x\to1^-}\mathrm{e}^{-\frac{1}{x^{2}}}\arctan\frac{x+1}{x-1}
      =-\frac{\pi}{2\mathrm{e}}$，$\lim_{x\to1^+}f(x)=\lim_{x\to1^+}\mathrm{e}^{-\frac{1}{x^{2}}}
  ...
  \section{综合题(每小题 6 分，共 24 分)}
  \begin{enumerate}
    \item[13.] \textbf{Solution}. 由连续函数的介值定理，存在$\eta_{1}\in(0,1),\eta
    ...
    \item[14.] \textbf{Solution}. $\frac{\mathrm{d}x}{\mathrm{d}y}=\frac{1}{\frac{\mathrm{d}y}{\mathrm{d}x}}
      =\frac{1}{y'}$.
    ...
    \item[15.] \textbf{Solution}. 令$f(x)=\arctan x-\frac{1}{2}\arccos\frac{2x}{1+x^{2}}
      (x\geq1)$，

      则$f'(x)=\frac{1}{1+x^{2}}+\frac{1}{2}\cdot\frac{\frac{2(1+x^{2})-4x^{2}}{(1+x^{2})^{2}}}{\sqrt{1-\left(\frac{2x}{1+x^2}\right)^{2}}}
      \equiv0$，所以$f(x)$在$[1,+\infty)$上恒为常数.

      又$f(1)=\arctan 1-\frac{1}{2}\arccos 1=\frac{\pi}{4}$，所以当$x\geq1$时，$\arctan
      x=\frac{\pi}{4}+\frac{1}{2}\arccos\frac{2x}{1+x^{2}}$.

    \item[16.] \textbf{Solution}. 记$b=a-f(a)>a$，则由Lagrange中值定理，存在$\xi\in
      (a,b)$使得
      \[
  ...
  \section{证明题(每小题 8 分，共 16 分)}
  \begin{enumerate}
    \item[17.] 数列$x_{n}$有界的定义：$\exists\,M>0$，$\forall\,n\in\mathbf{N}$，恒有$|
      x_{n}|\leq M$.
    ...
    \item[18.] \textbf{Proof}. （1）计算
      ...
      因此$\lim_{x\to0}\theta^{2}=\lim_{x\to0}\frac{x-\arctan x}{x^{2}\arctan x}=
      \lim_{x\to0}\frac{1-\frac{1}{1+x^{2}}}{3x^{2}}=\frac{1}{3}$，即$\lim_{x\to0}
      \theta=\frac{\sqrt{3}}{3}$.
  \end{enumerate}
\end{document}
