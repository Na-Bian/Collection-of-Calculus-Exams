...
\begin{document}
  ...
  \begin{enumerate}
    \item[11.] 设函数$f(x)$在$x=0$处连续，$f(0)\neq0$，且对一切$x,y$有$f(x+y)=f(x
      )\cdot f(y)$，证明$f(x)$处处连续.
      \vspace{13em}

    \item[12.] 讨论方程$\ln x-\frac{x}{\mathrm{e}}+1=0$有几个实根？并给出证明.
      \vspace{12em}

    \item[13.] 设函数$f(x)$在$x=2$处可导，且$f(2)\neq0$.求$\lim_{x\to\infty}\left
      [\frac{f\left(2+\frac{1}{x}\right)}{f(2)}\right]^{x}$.
      \vspace{12em}

    \item[14.] 有一个顶点朝上的圆锥形容器，高为$8\mathrm{m}$，底半径为$R=2\sqrt{2}
  ...
  \begin{enumerate}
    \item[15.] 证明不等式：当$0<x<1$时，$\frac{1-x}{1+x}<\mathrm{e}^{-2x}$.
      \vspace{12em}

    \item[16.] 设$0<x_{1}<x_{2}$，证明$x_{1}\ln x_{2}-x_{2}\ln x_{1}=(\ln\xi-1)(x
  ...
  \begin{enumerate}
    ...
    \item \textbf{Solution}. $\ln f(x) =\cos x\ln\sin x$，所以$f'(x)=\left(\sin x
    ...
    \item \textbf{Solution}. $f(x)=\frac{1}{2}-\frac{1}{2}\cdot\frac{1}{2x+1}$，所以$f
      ^{(n)}(x)=-\frac{1}{2}\cdot\frac{(-1)^{n}\cdot n!\,2^{n}}{(2x+1)^{n+1}}$，

      因此$f^{(n)}(0)=(-1)^{n-1}\cdot n!\cdot 2^{n-1}$.

    \item \textbf{Solution}. 因为$\lim_{x\to\infty}\frac{x\ln\left(\mathrm{e}+\frac{1}{x}\right)}{x}
      =1$，$\lim_{x\to\infty}x\ln\left(\mathrm{e}+\frac{1}{x}\right)-x=\lim_{t\to0}
  ...
  \section{综合题(每小题 7 分，共 28 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 在方程$f(x+y)=f(x)\cdot f(y)$中令$x=y=0$得$f(0)
      =f^{2}(0)$，因$f(0)\neq0$，所以$f(0)=1$.
    ...
    \item[12.] \textbf{Solution}. 设$f(x)=\ln x-\frac{x}{\mathrm{e}}+1$，则$f'(x)
    ...
    \item[13.] \textbf{Solution}.
      \[
        \begin{aligned}
          \lim_{x\to\infty}\left[\frac{f\left(2+\frac{1}{x}\right)}{f(2)}\right]^{x} = & \mathrm{e}^{\lim\limits_{x\to\infty}x\left[\ln f\left(2+\frac{1}{x}\right)-\ln f(2)\right]} \\
          =                                                                            & \mathrm{e}^{\lim\limits_{t\to0}\frac{\ln f(2+t)-\ln f(2)}{t}}                               \\
          =                                                                            & \mathrm{e}^{\left.\left(\ln f(t)\right)'\right|_{t=2}}=\mathrm{e}^{\frac{f'(2)}{f(2)}}.
        \end{aligned}
      \]

    \item[14.] \textbf{Solution}. 由几何关系，$V=\frac{1}{3}\cdot 8 \cdot 8\pi-\frac{1}{3}
  ...
  \section{证明题(每小题 6 分，共 12 分)}
  \begin{enumerate}
    \item[15.] \textbf{Proof}. 取对数，即证$\ln\frac{1-x}{1+x}<-2x(0<x<1)$.
    ...
    \item[16.] \textbf{Proof}. 设$f(x)=\frac{\ln x}{x}$，$g(x)=\frac{1}{x}$，则函数$f
