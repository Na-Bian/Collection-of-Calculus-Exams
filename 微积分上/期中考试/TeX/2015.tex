...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item[14.] 设$y=y(x)$由方程$y=\sin(x+y)$确定，求$\frac{\mathrm{d}^{2} y}{\mathrm{d}x^{2}}$.
      \vspace{9em}

    \item[15.] 设函数$f(x)$满足$f'(0)=0$，$f''(0)$存在，求极限$\lim_{x\to0}\frac{f(x)-f(\ln(1+x))}{x^{3}}$.
      \vspace{9em}

    \item[16.] 设$f(x)$是周期为5的连续函数，在$x=1$处可导，在$x=0$附近满足
      \[
    ...
    \item[17.] 设$x_{1}=2$，$x_{n}=2+\frac{1}{x_{n-1}}(n>1)$，证明$\lim_{n\to\infty}
      x_{n}$存在，并求其值.
      \vspace{10em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[18.] 设$f(x)$在闭区间$[a,b]$上有二阶导数，且$f(a)=f(b)=0$，$f'_{+}(a)\cdot
      f'_{-}(b)>0$，证明：
    ...
    \item[19.] 设函数$f(x)$在区间$[0,1]$上具有二阶导数，且有正常数$a,b$，使得$|f(
  ...
  \begin{enumerate}
    ...
    \item[14.] \textbf{Solution}. 在方程$y=\sin(x+y)$两边对$x$求导得
      \[
    ...
    \item[15.] \textbf{Solution}. 由于$f''(0)$存在，则$f(x)$在$x=0$的邻域内可导.
    ...
    \item[16.] \textbf{Solution}. 因为$f(x)$在$x=1$处可导，从而连续.
    ...
    \item[17.] \textbf{Solution}.显然$x_{n}>2$.常数$l=1+\sqrt{2}$满足$l=2+\frac{1}{l}$.下证数列$\{
      x_{n}\}$以$l$为极限.
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[18.] \textbf{Proof}. （1）假设$\forall\,x\in(a,b),f(x)\neq0$，不妨设$f(
      x)>0$，则有
      \[
        \begin{aligned}
