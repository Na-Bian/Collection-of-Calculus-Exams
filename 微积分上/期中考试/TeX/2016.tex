...
\begin{document}
  ...
  \begin{enumerate}
    \item[11.] 设函数$f(x)$在$x=a$处连续，$F(x)=(\mathrm{e}^{x}-\mathrm{e}^{a})f(
      x)$，计算导数$F'(a)$.
      \vspace{10em}
    ...
    \item[13.] 设函数$f(x)$在$x=1$处二阶可导，且$f(1+x)-3f(1-x)\sim 3x^{2}(x\to 0
      )$，求$f(1),f'(1),f''(1)$的值.
      \vspace{10em}

    \item[14.] 求无穷小量$u(x)=\arcsin x-\arctan x(x\to0)$的主部和阶数.
      \vspace{10em}

    \item[15.] 一根长为5米的竹竿斜靠着墙，地面与墙面垂直，竹竿在地面的投影也与墙面垂直.设墙面和地面是光滑的，使得竹竿顶端A沿着墙壁竖直往下滑动，同时，底端B沿着其投影线向外滑动.如果在底端B距离墙根为3米时，点B的速度为4米/秒，问此时顶端A下滑的速度为多少？
      \vspace{12em}
  \end{enumerate}

  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[16.] 由函数在一点可导可否推出它在该点的某个邻域内连续？认为可以请证明，认为不行请举反例.
      \vspace{12em}

    \item[17.] 设$f(x)$在$[0,1]$上连续，在$(0,1)$内可导，且$f(0)=0,f(1)=1$.设正实数$\lambda
  ...
  \begin{enumerate}
    ...
    \item[13.] \textbf{Solution}. 由题意可知$\lim_{x\to0}\left[f(1+x)-3f(1-x)\right
      ]=-2f(1)=0$，所以$f(1)=0$.
    ...
    \item[14.] \textbf{Solution}. 设$\lim_{x\to0}\frac{u(x)}{cx^{r}}=1$，则
      \[
        \begin{aligned}
    ...
    \item[15.] \textbf{Solution}. 设$t$时刻竹竿底端距离墙根的水平距离$x(t)$，竹竿顶端距离墙根的垂直距离为$y
      (t)$，则
      \[
        x^{2}(t)+y^{2}(t)=25.
      \]
  ...
  \begin{enumerate}
    ...
    \item[17.] \textbf{Proof}. 由于$f(x)$在$[0,1]$上连续，且$f(0)=0 < \lambda_{1}
      < 1=f(1)$，
