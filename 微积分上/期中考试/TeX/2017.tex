...
\begin{document}
  ...
  \begin{enumerate}
    \item[11.] 设函数$f(x)$在原点附近有界，$F(x)=f(x)\cdot\sin\left(x^{2}\right)$，计算导数$F
      '(0)$.
      \vspace{8em}
    ...
    \item[13.] 设函数$f(x)$在$x=0$的某个邻域内有界，满足$\lim_{x\to0}\frac{\sqrt[n]{1+f(x)\sin
      x}-1}{\mathrm{e}^{3x}-1}=2$，其中$n$为正整数.求$\lim_{x\to0}f(x)$.
      \vspace{9em}

    \item[14.] 求无穷小量$u(x)=x-\arctan x(x\to0)$的主部和阶数.
      \vspace{10em}

    \item[15.] 一个长方体的铁皮盒子，其对角线的长度随着长宽高的变化而连续变化.当长宽高分别是$3
  ...
  \begin{enumerate}
    ...
    \item[17.] 设$f(x)$在$[0,3]$上连续，在$(0,3)$内一阶可导，且$f(0)=0,f(1)=3,f(3
  ...
  \begin{enumerate}
    ...
    \item[13.] \textbf{Solution}. 由于$f(x)$在$x=0$的某个邻域内有界，所以$\lim_{x\to0}
      f(x)\sin x=0$，故
      \[
        \begin{aligned}
    ...
    \item[15.] \textbf{Solution}. 设$t$时刻长方体的长宽高及对角线长分别为$x(t),y(
      t),z(t),s(t)$，则
      \[
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[16.] \textbf{Proof}. 计算$\lim_{x\to0}\frac{f(x)-0}{x-0}=
      \begin{cases}
    ...
    \item[17.] \textbf{Proof}. 由于$f(x)$在$[0,3]$上连续，由介值定理，存在$\eta\in
