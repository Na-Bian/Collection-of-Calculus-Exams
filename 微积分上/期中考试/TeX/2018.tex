...
\begin{document}
  ...
  \begin{enumerate}
    \item[11.] 讨论函数$f(x)=\frac{\sin\pi x}{x^{2}-1}$的连续性，并对函数的间断点判别类型.
      \vspace{8em}

    \item[12.] 求无穷小量$u(x)=\left(\frac{1+2\cos x}{3}\right)^{x^2}-1(x\to0)$的主部和阶数.
      \vspace{8em}
    ...
    \item[14.] 已知$a>1$，$n\geq1$，证明不等式$\frac{\sqrt[n]{a}-\sqrt[n+1]{a}}{\ln
      a}<\frac{\sqrt[n]{a}}{n^{2}}$.
      \vspace{10em}

    \item[15.] 一架飞机在$H$米高空以$a$米/秒的速度水平匀速飞行.设在$t=0$时刻有一探照灯位于飞机正下方的地面上跟踪飞机.问$t$秒以后探照灯应以怎样的角速度转动才能照到飞机？
      \vspace{12em}
  \end{enumerate}

  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[16.] 设数列$\{x_{n}\}$由递推公式$x_{1}=1$，$x_{n+1}=x_{n}+\frac{2}{x_{n}^{2}}$给出.证明数列$\{
      x_{n}\}$无界.
      \vspace{12em}

    \item[17.] 设$f(x)$在区间$[a,b]$有定义并且对于任何$x,y\in[a,b](x\neq y)$成立
      \[
        |f(x)-f(y)|\leq M(x-y)^{2},
      \]
  ...
  \section{综合题(每小题 6 分，共 30 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 函数$f(x)$的间断点为$-1,1$.因
      \[
        \begin{aligned}
    ...
    \item[13.] \textbf{Solution}. 当$x>0$时，$f'(x)=\frac{1}{1+x}$；当$x<0$时，$f
      '(x)=1$；
    ...
    \item[14.] \textbf{Solution}. 令$f(x)=a^{x}$，显然$f(x)$在区间$\left[\frac{1}{n+1}
    ...
    \item[15.] \textbf{Solution}. 设飞机飞过的距离为$x$米，飞机与探照灯的连线与竖直方向的夹角为$\theta$.
  ...
  \begin{enumerate}
    ...
    \item[17.] \textbf{Proof}. 由题可知，$\forall\,x,y\in[a,b](x\neq y)$，有$0\leq
