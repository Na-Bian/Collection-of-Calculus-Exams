...
\begin{document}
  ...
  \begin{enumerate}
    \item 设数列$\{x_{n}\}$收敛，则【\quad】.
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 当$\lim_{n\to\infty}\sin x_{n}=0$时，$\lim_{n\to\infty}x_{n}=0$                   & \textbf{B.} 当$\lim_{n\to\infty}\left(x_{n}+\sqrt{|x_{n}|}\right)=0$时，$\lim_{n\to\infty}x_{n}=0$ \\
        \textbf{C.} 当$\lim_{n\to\infty}\left(x_{n}+x_{n}^{2}\right)=0$时，$\lim_{n\to\infty}x_{n}=0$ & \textbf{D.} 当$\lim_{n\to\infty}\left(x_{n}+\sin x_{n}\right)=0$时，$\lim_{n\to\infty}x_{n}=0$
      \end{tabular}

    \item 极限$\lim_{x\to\infty}\left[\frac{x^{2}}{(x-a)(x+b)}\right]^{x}$=【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $1$                & \textbf{B.} $\mathrm{e}$       \\
        \textbf{C.} $\mathrm{e}^{a-b}$ & \textbf{D.} $\mathrm{e}^{b-a}$
      \end{tabular}
    ...
    \item 设$F(x)=
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 连续点   & \textbf{B.} 可去间断点 \\
        \textbf{C.} 无穷间断点 & \textbf{D.} 跳跃间断点
      \end{tabular}

    \item 函数$f(x)$在$x=0$处可导，且$f(0)=0$，则$\lim_{x\to0}\frac{x^{2}f(x)-2f(x^{3})}{x^{3}}$=【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $-2f'(0)$ & \textbf{B.} $-f'(0)$ \\
        \textbf{C.} $f'(0)$   & \textbf{D.} $0$
      \end{tabular}
    ...
    \item 设$f(x)$具有一阶连续导数，$F(x)=f(x)\left(1+|\sin x|\right)$，则$f(0)=0$是$F
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 充分必要条件     & \textbf{B.} 充分条件但非必要条件 \\
        \textbf{C.} 必要条件但非充分条件 & \textbf{D.} 既非充分又非必要条件
      \end{tabular}

    \item 设$f(x)$有二阶连续导数，且$f'(0)=0$，$\lim_{x\to0}\frac{f''(x)}{|x|}=1$，则【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $f(0)$是$f(x)$的极大值                   & \textbf{B.} $f(0)$是$f(x)$的极小值  \\
        \textbf{C.} $\left(0,f(0)\right)$是曲线$y=f(x)$的拐点 & \textbf{D.} $f'(0)$是$f'(x)$的极值
      \end{tabular}
  \end{enumerate}

  \section{填空题(每小题 4 分，共 16 分)}
  \begin{enumerate}
    \item[7.] 极限$\lim_{x\to0}x\left[\frac{1}{x}\right]=\underline{\hspace{3cm}}$.

    \item[8.] 设$y=\frac{\cos x}{x^{2}}$，则$\frac{\mathrm{d}y}{\mathrm{d}(\cos
      x)}=\underline{\hspace{3cm}}$.

    \item[9.] $\lim_{x\to0}\left(\frac{1}{\mathrm{e}^{x}-1}-\frac{1}{\ln(1+x)}\right
      )=\underline{\hspace{3cm}}$.

    \item[10.] 曲线$y=x\left(1+\arcsin\frac{2}{x}\right)$的斜渐近线方程为\underline{\hspace{3cm}}.
  \end{enumerate}

  \section{基本计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] 设$f(x)=x+a\ln(1+x)+bx\sin x$，$g(x)=cx^{3}$，在$x\to0$时，若$f(x)$与$g
    ...
    \item[14.] 设$y=\frac{x^{2}}{2-x}$，计算$y^{(50)}(0)$.
      \vspace{10em}

    \item[15.] 设$x=\varphi(y)$是函数$y=x\ln x$的反函数，计算$\varphi(y)$在$x=\mathrm{e}$处的二阶导数$\frac{\mathrm{d}^{2}
      x}{\mathrm{d}y^{2}}$.
      \vspace{12em}

    \item[16.] 求$f(x)=x^{3}-3x^{2}-9x+5$在区间$[-2,2]$上的最大值与最小值.
      \vspace{12em}
  \end{enumerate}

  \section{应用题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] 如果以每秒$50\mathrm{cm}^{3}$的速度匀速给一个气球充气，假设气球内气压保持常值且形状始终为球形，问当气球的半径为$5
      \mathrm{cm}$时，气球半径增加的速率是多少？
      \vspace{15em}

    \item[18.] 设在$\left(-\infty,+\infty\right)$内$f''(x)>0$，$f(0)<0$，讨论$F(x
      )=\frac{f(x)}{x}(x\neq0)$的单调性.
      \vspace{15em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[19.] 设$x_{n}=\sqrt{a+\sqrt{a+\cdots+\sqrt{a}}}$（$n$个根式，$a>0$），证明$\{
      x_{n}\}$收敛，并求出极限值.
      \vspace{18em}

    \item[20.] 设函数$f(x)$在区间$[0,2]$上具有连续导数，$f(0)=f(2)=0$，$M=\max_{x\in(0,2)}
  ...
  \begin{enumerate}
    ...
    \item[8.] \textbf{Solution}. $\frac{x\sin x+2\cos x}{x^{3}\sin x}$.
  ...
  \begin{enumerate}
    ...
    \item[13.] \textbf{Solution}. $\frac{\mathrm{d}y}{\mathrm{d}x}=\frac{\frac{\mathrm{d}y}{\mathrm{d}t}}{\frac{\mathrm{d}x}{\mathrm{d}t}}
    ...
    \item[14.] \textbf{Solution}. $y=\frac{x^{2}}{2-x}=\frac{(2-x)^{2}-4(2-x)+4}{2-x}
      =\frac{4}{2-x}-2-x$.
    ...
    \item[15.] \textbf{Solution}. $\frac{\mathrm{d}x}{\mathrm{d}y}=\frac{1}{\ln
    ...
    \item[16.] \textbf{Solution}. $f'(x)=3x^{2}-6x-9=3(x+1)(x-3)$，故$f(x)$在$[-2
      ,2]$内的极值点为$x=-1$.
  ...
  \section{应用题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] \textbf{Solution}. 设气球的半径为$r$，体积为$V$，则$V=\frac{4}{3}\pi r
      ^{3}$.
    ...
    \item[18.] \textbf{Solution}. $F'(x)=\frac{xf'(x)-f(x)}{x^{2}}$，令$G(x)=xf'(x)-
      f(x)$，则$G'(x)=xf''(x)$.
  ...
  \section{综合题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] \textbf{Proof}. 由题可知$x_{1}=\sqrt{a}$，$x_{n+1}=\sqrt{a+x_{n}}$.
    ...
    \item[20.] \textbf{Proof}. 假设$\forall\,x\in(0,2)$，$|f'(x)|<M$.
