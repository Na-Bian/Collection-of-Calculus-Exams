...
\begin{document}
  ...
  \begin{enumerate}
    \item 设$0<a_{n}<1$，且$\lim_{n\to\infty}a_{n}=1$，则以下数列中无界的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} $\left\{a_{n}^{2}\right\}$ & \textbf{B.} $\left\{\frac{1}{a_{n}}\right\}$ & \textbf{C.} $\left\{\tan\frac{\pi a_{n}}{2}\right\}$ & \textbf{D.} $\left\{\ln a_{n}\right\}$
      \end{tabular}
      \vspace{1em}

    \item 已知$f(2)=3$，$f'(2)=5$，则极限$\lim_{h\to0}\frac{f^{2}(2+h)-9}{h}=$【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} $30$ & \textbf{B.} $10$ & \textbf{C.} $6$ & \textbf{D.} $0$
      \end{tabular}
      \vspace{1em}

    \item 设函数$f(x)$在闭区间$[a,b]$上可导，则以下说法中\textbf{错误}的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $f(x)$必在$[a,b]$上有界 & \textbf{B.} $f(x)$必在$[a,b]$上有连续的导数 \\
        \textbf{C.} $f(x)$必在$[a,b]$上连续 & \textbf{D.} $f(x)$必在$[a,b]$上可积
      \end{tabular}
      \vspace{1em}

    \item 曲线$y=\frac{1}{x}+\ln\left(1+\mathrm{e}^{x}\right)$一共有【\quad】条渐近线.
    ...
    \item 设$f(x)$二阶可导，且$f''(x)<0$，则以下不等式中一定成立的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $f(1)+f(3)>2f(2)$ & \textbf{B.} $f(1)+f(3)<2f(2)$ \\
        \textbf{C.} $f(1)+f(2)>2f(3)$ & \textbf{D.} $f(1)+f(2)<2f(3)$
      \end{tabular}
      \vspace{1em}

    \item 微分方程$y'-\frac{y}{2x}=0$满足初值条件$y(1)=2$的特解为【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} $2\sqrt{x}$ & \textbf{B.} $1+\sqrt{x}$ & \textbf{C.} $1+x$ & \textbf{D.} $\sqrt{x+3}$
      \end{tabular}
      \vspace{1em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[7.] 极限$\lim_{n\to\infty}\frac{1}{n^{4}}\left(1^{3}+2^{3}+\cdots+n^{3}
      \right)=\underline{\hspace{3cm}}$.
      \vspace{0.5em}

    \item[8.] 设函数$y=x^{2}-x$. 在$x=2$，$\Delta x=0.01$时，微分$\mathrm{d}y=\underline
      {\hspace{3cm}}$.
      \vspace{0.5em}

    \item[9.] $\lim_{x\to0}\frac{\displaystyle\int_{0}^{\sin^2 x}\sqrt{3+t^2}\mathrm{d}t}{x^{2}}
      =\underline{\hspace{3cm}}$.
      \vspace{0.5em}

    \item[10.] 曲线$y=1-x^{4}$与$x$轴所围成图形的面积为$\underline{\hspace{3cm}}$.
  \end{enumerate}

  \section{计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] 已知当$x\to0$时，$u=\mathrm{e}^{3x}-ax^{2}-(1+bx)\cos x$是与$x^{3}$同阶的无穷小.
      求常数$a,b$的值.
      \vspace{12em}

    \item[12.] 求$f(x)=x^{3}-3x^{2}-9x+5$在区间$[-2,2]$上的最大值与最小值.
      \vspace{12em}

    \item[13.] 求不定积分$I=\int \frac{\mathrm{e}^{5x}}{\mathrm{e}^{2x}+1}\mathrm{d}
      x$.
      \vspace{11em}

    \item[14.] 求定积分$I=\int_{0}^{\frac{\pi}{4}}\frac{x}{\cos^{2} x}\mathrm{d}x$.
      \vspace{11em}

    \item[15.] 判定反常积分$\int_{0}^{1}\frac{1}{x\sqrt{x^{2}+1}}\mathrm{d}x$的敛散性，若收敛求其值.
      \vspace{12em}

    \item[16.] 求微分方程$y''=\frac{\sin y}{\cos^{3} y}$满足初值条件$y(2)=0$，$y'
      (2)=1$的特解.
      \vspace{12em}
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item[18.] 求曲线$y=\sqrt{x}-\frac{x\sqrt{x}}{3}$对应于$1\leq x\leq 4$弧段的长度.
      \vspace{15em}
  \end{enumerate}

  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] 设$n$为正整数，求方程$(x+1)^{2n}=x^{2n}+1$所有的实根.证明你的结论.
      \vspace{15em}

    \item[20.] 设函数$f(x)$在区间$[a,b]$上有连续的导数. 证明：
      \[
  ...
  \section{填空题(每小题 4 分，共 16 分)}
  \begin{enumerate}
    \item[7.] \textbf{Solution}. $\frac{1}{4}$.
    ...
    \item[10.] \textbf{Solution}. $\frac{8}{5}$.

      所围成图形的面积$S=\int_{-1}^{1}\left(1-x^{4}\right)\mathrm{d}x = \frac{8}{5}$.
  \end{enumerate}
  \section{计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 由Taylor公式，
      \[
        \begin{aligned}
    ...
    \item[12.] \textbf{Solution}. $f'(x)=3x^{2}-6x-9$，令$f'(x)=0$得到函数在$(-2,
      2)$内的驻点为$x=-1$.

      计算$f(-2)=3$，$f(-1)=10$，$f(2)=-17$，所以函数在区间$[-2,2]$上的最大值为$1
      0$，最小值为$-17$.

    \item[13.] \textbf{Solution}. 令$u=\mathrm{e}^{x}$，则$\mathrm{d}u=\mathrm{e}
      ^{x}\mathrm{d}x$，
      \[
        \begin{aligned}
    ...
    \item[15.] \textbf{Solution}. 令$x=\tan t$，则$\mathrm{d}x=\sec^{2} t\mathrm{d}
      t$，
      \[
        \begin{aligned}
    ...
    \item[16.] \textbf{Solution}. 令$y'=p$，则$y''=\frac{\mathrm{d}p}{\mathrm{d}x}
  ...
  \section{综合题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] \textbf{Solution}. 当$x\in(-1,0)\cup(0,+\infty)$时，$f'(x)=\frac{\frac{x}{1+x}-\ln(1+x)}{x^{2}}
    ...
    \item[18.] \textbf{Solution}.$y'=\frac{1}{2\sqrt{x}}-\frac{\sqrt{x}}{2}$.
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] \textbf{Proof}. 显然$x=0$是方程$(x+1)^{2n}=x^{2n}+1$的一个实根.
    ...
    \item[20.] \textbf{Solution}. 根据积分中值定理，$\exists\,\xi\in[a,b]$，使得$f
