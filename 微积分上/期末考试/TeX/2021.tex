...
\begin{document}
  ...
  \begin{enumerate}
    \item 设函数$f(x)$在$(-\infty,+\infty)$内单调有界，$\{x_{n}\}$是一数列，则下列命题正确的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 若$\{x_{n}\}$收敛，则$\{f(x_{n})\}$收敛 & \textbf{B.} 若$\{x_{n}\}$单调，则$\{f(x_{n})\}$收敛 \\
        \textbf{C.} 若$\{f(x_{n})\}$收敛，则$\{x_{n}\}$收敛 & \textbf{D.} 若$\{f(x_{n})\}$单调，则$\{x_{n}\}$收敛
      \end{tabular}
      \vspace{1em}

    \item 函数$f(x)=\lim_{n\to\infty}\frac{x^{n}+2}{x^{n}+1}$的间断点及类型是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $x=-1$是第二类间断点    & \textbf{B.} $x=1$是第二类间断点      \\
        \textbf{C.} $x=\pm1$均是第一类间断点 & \textbf{D.} $x=\pm 1$均是第二类间断点
      \end{tabular}
      \vspace{1em}

    \item $x\to0^{+}$时，与$\sqrt{x}$等价的无穷小量是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} $1-\mathrm{e}^{\sqrt{x}}$ & \textbf{B.} $\sqrt{1+\sqrt{x}}-1$ & \textbf{C.} $\ln\frac{1+x}{1-\sqrt{x}}$ & \textbf{D.} $1-\cos{\sqrt{x}}$
      \end{tabular}
      \vspace{1em}

    \item 设函数$f(x)$在$x=0$处连续，下列命题错误的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 若$\lim_{x\to0}\frac{f(x)}{x}$存在，则$f(0)=0$  & \textbf{B.} 若$\lim_{x\to0}\frac{f(x)+f(-x)}{x}$存在，则$f(0)=0$  \\
        \textbf{C.} 若$\lim_{x\to0}\frac{f(x)}{x}$存在，则$f'(0)$存在 & \textbf{D.} 若$\lim_{x\to0}\frac{f(x)-f(-x)}{x}$存在，则$f'(0)$存在
      \end{tabular}
      \vspace{1em}

    \item 曲线$y=x\ln\left(\mathrm{e}+\frac{1}{x}\right)(x>0)$的渐近线条数为【\quad】.
    ...
    \item 设$F(x)=\int^{x+2\pi}_{x}\mathrm{e}^{\sin t}\sin t\mathrm{d}t$，则$F(x)$【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} 为正常数 & \textbf{B.} 为负常数 & \textbf{C.} 恒为0 & \textbf{D.} 不为常数
      \end{tabular}
      \vspace{1em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[7.] 曲线$\begin{cases}
        x = \arctan t,      \\
        y=\ln\sqrt{1+t^{2}}
      \end{cases}$对应于$t=1$处的法线方程为$=\underline{\hspace{3cm}}$.
      \vspace{0.5em}

    \item[8.] 曲线$y=x\sin x+2\cos x\left(-\frac{\pi}{2}<x<2\pi\right)$的拐点是$\underline
      {\hspace{3cm}}$.
      \vspace{0.5em}

    \item[9.] 曲线$y=\ln\cos x\left(0\leq x\leq\frac{\pi}{6}\right)$的弧长为$\underline
      {\hspace{3cm}}$.
      \vspace{0.5em}

    \item[10.] $y=2^{x}$的Maclaurin展开式中$x^{n}$项的系数为$\underline{\hspace{3cm}
      }$.
  \end{enumerate}

  \section{计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] 已知$\lim_{x\to1}\frac{ax^{2}+x-3}{x-1}=b$，求常数$a,b$的值.
      \vspace{12em}

    \item[12.] 设$f(x)$为连续函数，且满足$f(x)=x^{2}-x\cdot f(2)+2\int_{0}^{1}f(x
      )\mathrm{d}x$，求$f(x)$.
      \vspace{11em}

    \item[13.] 求极限$l=\lim_{n\to\infty}\sum_{i=0}^{n-1}\frac{1}{n+i}$.
      \vspace{11em}

    \item[14.] 计算定积分$I=\int_{0}^{1}x^{3}\sqrt{1-x^{2}}\mathrm{d}x$.
      \vspace{11em}
    ...
    \item[16.] 求微分方程$xy'+y-\mathrm{e}^{x}=0$，$y(2)=1$的特解.
      \vspace{12em}
  \end{enumerate}

  \section{综合题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] 已知$f(x)$在$(-\infty,+\infty)$上连续，$f(x)=(x+1)^{2}+2\int_{0}^{x}
      f(t)\mathrm{d}t$，求$f^{(n)}(0)$的值（$n\geq2$）.
      \vspace{12em}

    \item[18.] 设抛物线$y=ax^{2}+bx+c$过原点，当$0\leq x\leq 1$时，$y\geq0$，又该抛物线与直线$x
  ...
  \begin{enumerate}
    \item[19.] 证明方程$\ln x = \frac{x}{\mathrm{e}}-2021$在区间$(0,+\infty)$内只有两个不同的实根.
      \vspace{14em}

    \item[20.] 设$f''(x)$在$[0,2]$上连续，且$|f''(x)|\leq M$，$f(1)=0$. 证明：
      \[
  ...
  \begin{enumerate}
    ...
    \item \textbf{Solution}. C.
      ...
      \begin{cases}
        ...
        \frac{3}{2}, & x=1,   \\
        \text{不存在},  & x=-1
      \end{cases}$，因此$x=\pm1$均为$f(x)$的跳跃间断点，即第一类间断点.
    ...
    \item \textbf{Solution}. D.
      ...
      对于D选项，考虑$f(x)=|x|$，则
      \[
        \lim_{x\to0^-}\frac{f(x)-f(-x)}{x}=\lim_{x\to0^-}\frac{(-x)-(-x)}{x}= 0 \text{，}
    ...
    \item \textbf{Solution}. B.
      ...
      当$x\to0^{+}$时，$\lim_{x\to0^+}x\ln\left(\mathrm{e}+\frac{1}{x}\right)=\lim
      _{t\to+\infty}\frac{\ln(\mathrm{e}+t)}{t}=0$，所以曲线$y=x\ln\left(\mathrm{e}
      +\frac{1}{x}\right)$没有竖直渐近线.

      当$x\to+\infty$时，$\lim_{x\to+\infty}\frac{x\ln\left(\mathrm{e}+\frac{1}{x}\right)}{x}
      =1$，$\lim_{x\to+\infty}\left[x\ln\left(\mathrm{e}+\frac{1}{x}\right)-x\right
      ]=\lim_{t\to0^+}\frac{\ln(\mathrm{e}+t)-1}{t}=\frac{1}{\mathrm{e}}$，

      所以曲线$y=x\ln\left(\mathrm{e}+\frac{1}{x}\right)$有斜渐近线$y=x+\frac{1}{\mathrm{e}}$.

      故曲线$y=x\ln\left(\mathrm{e}+\frac{1}{x}\right)$的渐近线条数为1.
    ...
    \item \textbf{Solution}. A.
      ...
      对第二项作代换$u=2\pi-t$，则$\int_{\pi}^{2\pi}\mathrm{e}^{\sin t}\sin t\mathrm{d}
      ...
      因为在$(0,\pi)$上，$\sin t>0$，且$\mathrm{e}^{\sin t}>\mathrm{e}^{-\sin t}$，所以$\int
      _{0}^{\pi}\left(\mathrm{e}^{\sin t}-\mathrm{e}^{-\sin t}\right)\sin t\mathrm{d}
      t=\int_{0}^{2\pi}\mathrm{e}^{\sin t}\sin t\mathrm{d}t>0$.
  \end{enumerate}

  \section{填空题(每小题 4 分，共 16 分)}
  \begin{enumerate}
    \item[7.] \textbf{Solution}. $y+x-\frac{1}{2}\ln 2-\frac{\pi}{4}=0$.

      当$t=1$时，$x=\arctan 1=\frac{\pi}{4}$，$y=\ln\sqrt{2}=\frac{1}{2}\ln 2$.

      $\frac{\mathrm{d}y}{\mathrm{d}x}=\frac{\frac{\mathrm{d}y}{\mathrm{d}t}}{\frac{\mathrm{d}x}{\mathrm{d}t}}
      =\frac{\frac{2t}{2(1+t^{2})}}{\frac{1}{1+t^{2}}}=t$，所以$\left.\frac{\mathrm{d}y}{\mathrm{d}x}
      \right|_{t=1}=1$.

      切线斜率为1，故法线斜率为$-1$，法线方程为$y-\frac{1}{2}\ln 2=-1\cdot\left(x
      -\frac{\pi}{4}\right)$，即$y+x-\frac{1}{2}\ln 2-\frac{\pi}{4}=0$.

    \item[8.] \textbf{Solution}. $(\pi,-2)$.

      $y'=\sin x+x\cos x-2\sin x$，$y''=\cos x+\cos x-x\sin x-2\cos x=-x\sin x$，
    ...
    \item[9.] \textbf{Solution}. $\frac{1}{2}\ln 3$.

      $y'=\frac{-\sin x}{\cos x}=-\tan x$，所以$\mathrm{d}s=\sqrt{1+y'^{2}}\mathrm{d}
      x=\sqrt{1+\tan^{2}x}\mathrm{d}x=\sec x\mathrm{d}x$，

      故$s=\int_{0}^{\frac{\pi}{6}}\sec x\mathrm{d}x=\ln\left|\sec x+\tan x\right
      |\bigg|_{0}^{\frac{\pi}{6}}=\ln\left(\frac{2}{\sqrt{3}}+\frac{1}{\sqrt{3}}\right
      )=\frac{1}{2}\ln 3$.

    \item[10.] \textbf{Solution}. $\frac{(\ln 2)^{n}}{n!}$.

      $y=2^{x}=\mathrm{e}^{x\ln 2}=\sum_{n=0}^{\infty}\frac{(x\ln 2)^{n}}{n!}$，所以$x
      ^{n}$项的系数为$\frac{(\ln 2)^{n}}{n!}$.
  \end{enumerate}
  \section{计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 由题可知$\lim_{x\to1}\left(ax^{2}+x-3\right)=a+
      1-3=0$，所以$a=2$.

      $b=\lim_{x\to1}\frac{2x^{2}+x-3}{x-1}=\lim_{x\to1}\frac{(2x+3)(x-1)}{x-1}=5$.

    \item[12.] \textbf{Solution}. 记$\int_{0}^{1}f(x)\mathrm{d}x=C$.
      ...
      解得$C=\int_{0}^{1}f(x)\mathrm{d}x=\frac{1}{2}$，所以$f(x)=x^{2}-x\cdot f(2
      )+2\int_{0}^{1}f(x)\mathrm{d}x=x^{2}-\frac{5}{3}x+1$.

    \item[13.] \textbf{Solution}. 由定积分的定义，
      \[
        \begin{aligned}
    ...
    \item[14.] \textbf{Solution}. 令$x=\sin\theta$，则$\mathrm{d}x=\cos\theta\mathrm{d}
      \theta$，
    ...
    \item[16.] \textbf{Solution}. 方程变形为$y'+\frac{1}{x}y=\frac{\mathrm{e}^{x}}{x}$.
  ...
  \section{综合题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] \textbf{Solution}. 由于$f(x)$连续，所以其积分变上限函数$\int_{0}^{x}
      f(t)\mathrm{d}t$可导，
      ...
      方程$f(x)=(x+1)^{2}+2\int_{0}^{x}f(t)\mathrm{d}t$两边令$x=0$得$f(0)=1$，所以$C
      =1+\frac{3}{2}=\frac{5}{2}$，$f(x)=\frac{5}{2}\mathrm{e}^{2x}-x-\frac{3}{2}$.

      当$n\geq2$时，$f^{(n)}(0)=\left.\left(\frac{5}{2}\mathrm{e}^{2x}\right)^{(n)}
      \right|_{x=0}=5\cdot 2^{n-1}\mathrm{e}^{2x}\bigg|_{x=0}=5\cdot 2^{n-1}$.

    \item[18.] \textbf{Solution}. 由抛物线过原点得$c=0$，且有
      \[
        \frac{1}{3}=\int_{0}^{1}\left(ax^{2}+bx\right)\mathrm{d}x=\frac{a}{3}+\frac{b}{2}
        ,
      \]

      即$b=\frac{2}{3}(1-a)$. 旋转体的体积$V=$
      \[
        \begin{aligned}
          \int_{0}^{1}\pi f^{2}(x)\mathrm{d}x = & \pi\int_{0}^{1}\left(ax^{2}+bx\right)^{2}\mathrm{d}x = \pi\int_{0}^{1}x^{2}(ax+b)^{2}\mathrm{d}x                                             \\
      ...
      \right]=0$解得$a=-\frac{5}{4}$，又$\left.\frac{\mathrm{d}^{2}V}{\mathrm{d}a^{2}}
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] \textbf{Proof}. 令$f(x)=\ln x-\frac{x}{\mathrm{e}}+2021$，则$f'(x)
    ...
    \item[20.] \textbf{Solution}. 由Taylor公式，将$f(x)$在$x=1$处展开得
      \[
        \begin{gathered}
