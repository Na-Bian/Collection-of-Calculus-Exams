...
\begin{document}
  ...
  \begin{enumerate}
    \item 若$f'(x_{0})=\frac{1}{2}$，则当$\Delta x\to0$时，函数$y=f(x)$在$x=x_{0}$处的微分$\mathrm{d}
      y$是与$\Delta x$【\quad】的无穷小.

      \begin{tabular}{l@{\hspace{2cm}}l@{\hspace{2cm}}l@{\hspace{2cm}}l}
        \textbf{A.} 高阶 & \textbf{B.} 低阶 & \textbf{C.} 同阶 & \textbf{D.} 等价
      \end{tabular}

    \item 设$f(x)$在$x=0$处连续，则$f(x)$在$x=0$处可导的充分条件是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $\lim_{x\to0}\frac{f(x)-f(-x)}{2x}$存在         & \textbf{B.} $\lim_{x\to0}\frac{f(x^{2})-f(0)}{x^{2}}$存在      \\
        \textbf{C.} $\lim_{x\to0}\frac{f(x)-f(0)}{\sqrt[3]{x}}$存在 & \textbf{D.} $\lim_{x\to\infty}x f\left(\frac{1}{x}\right)$存在
      \end{tabular}

    \item 下列关于数列的描述中，正确的是【\quad】.

      \begin{tabular}{l}
        \textbf{A.} 若$\lim_{n\to\infty}x_{n} y_{n}=0$，则必有$\lim_{n\to\infty}x_{n}=0$或$\lim_{n\to\infty}y_{n}=0$ \\
        \textbf{B.} 若$\{x_{n} y_{n}\}$有界，则必有$\{x_{n}\}$有界或$\{y_{n}\}$有界                                        \\
        \textbf{C.} 若$\lim_{n\to\infty}(x_{n}-y_{n})=0$，则$\lim_{n\to\infty}x_{n}=\lim_{n\to\infty}y_{n}$       \\
        \textbf{D.} 若有区间$\mathrm{I}$内的数列$x_{n}$，使$|f(x_{n})|$无界，则$f(x)$在$\mathrm{I}$内无界
      \end{tabular}
    ...
    \item 设$f(x)$在区间$(a,+\infty)$内可导，且$f'(x)>0$，若$f(a)=0$，则在区间$(a
      ...
      \begin{tabular}{l@{\hspace{1.4cm}}l@{\hspace{1.4cm}}l@{\hspace{1.4cm}}l}
        \textbf{A.} $f(x)\geq0$ & \textbf{B.} $f(x)>0$ & \textbf{C.} 不能确定$f(x)$的符号 & \textbf{D.} $f(x)$单调趋向于$+\infty$
      \end{tabular}

    \item 已知$f(x)$在$x=0$处连续，且$\lim_{x\to0}\frac{f(x)}{x^{2}}=1$，则下列结论成立的是【\quad】.

      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $f'(0)$存在，且$f'(0)\neq0$ & \textbf{B.} $f'(0)$不存在         \\
        \textbf{C.} $f(x)$在$x=0$处取得极小值      & \textbf{D.} $f(x)$在$x=0$处取得极大值
      \end{tabular}
    ...
    \item $\lim_{n\to\infty}\ln\sqrt[n]{\left(1+\frac{1^{2}}{n^{2}}\right)\left(1+\frac{2^{2}}{n^{2}}\right)\cdots\left(1+\frac{n^{2}}{n^{2}}\right)}
      ...
      \begin{tabular}{l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l}
        \textbf{A.} $\int_{1}^{2}\ln(1+x^{2})\mathrm{d}x$ & \textbf{B.} $2\int_{1}^{2}\ln x\mathrm{d}x$ & \textbf{C.} $\int_{0}^{1}\ln(1+x^{2})\mathrm{d}x$ & \textbf{D.} $2\int_{0}^{1}\ln x\mathrm{d}x$
      \end{tabular}
  \end{enumerate}

  \section{填空题(每小题 4 分，共 16 分)}
  \begin{enumerate}
    \item[7.] 当$x\to0$时，无穷小量$\left(1+x\right)^{x^2}-1$的阶数是$=\underline
      {\hspace{3cm}}$.
      \vspace{0.5em}

    \item[8.] 曲线$\cot\left(x+y+\frac{\pi}{4}\right)=\mathrm{e}^{y}$在点$(0,0)$处的切线方程为$\underline
      {\hspace{3cm}}$.
      \vspace{0.5em}

    \item[9.] 若$\int_{0}^{x}f(t)\mathrm{d}t=x\mathrm{e}^{-x}$，则$\int_{1}^{+\infty}
      \frac{f(\ln x)}{x}\mathrm{d}x=\underline{\hspace{3cm}}$.
      \vspace{0.5em}

    \item[10.] 曲线$y=\int_{0}^{x}\tan t\mathrm{d}t\left(0\leq x\leq \frac{\pi}{4}
  ...
  \begin{enumerate}
    \item[11.] 求曲线$C:y=x\arctan x(x>0)$的渐近线.
      \vspace{12em}

    \item[12.] 设$f(x)$在$x=a$的某邻域内可导，且$f(a)=f'(a)=1$，求$l=\lim_{x\to a}
      \left(\frac{1}{x-a}-\frac{1}{\displaystyle\int_{a}^{x}f(t)\mathrm{d}t}\right
      )$.
      \vspace{11em}

    \item[13.] 求$I=\int\frac{1}{x(x^{n}+1)}\mathrm{d}x$，其中$n$为正整数.
      \vspace{11em}

    \item[14.] 求微分方程$\frac{\mathrm{d}y}{\mathrm{d}x}=\frac{y}{2x+y^{4}}$的通解.
      \vspace{11em}

    \item[15.] 设$f(x)=\int_{x}^{1}\sin t^{2}\mathrm{d}t$，计算$I=\int_{0}^{1}f(x
      )\mathrm{d}x$.
      \vspace{12em}

    \item[16.] 求$F(x)=\int_{-\frac{\pi}{2}}^{x}\mathrm{e}^{-t}\sin t\mathrm{d}t$在区间$\left
  ...
  \begin{enumerate}
    \item[17.] 已知函数$f(x)=\int_{1}^{x}\sqrt{1+t^{2}}\mathrm{d}t+\int_{x^2}^{1}
      \sqrt{1+t}\mathrm{d}t$，讨论方程$f(x)=0$的实根个数.
      \vspace{12em}

    \item[18.] 设$f(x)$满足$\int_{0}^{x}f(t-x)\mathrm{d}t=-\frac{x^{3}}{3}-x^{2}$，求曲线$y
  ...
  \begin{enumerate}
    \item[19.] 设函数$f(x),g(x)$在$[0,1]$上连续，且$f(x)>0$，$g(x)\geq0$.证明：
      \[
    ...
    \item[20.] 设$f(x)$在$[0,2]$上二阶可导，且$f'(0)=f'(2)=0$. 试证：存在$\xi\in(
      0,2)$，使
      \[
        |f''(\xi)|\geq|f(2)-f(0)|.
      \]
  ...
  \begin{enumerate}
    ...
    \item[10.] \textbf{Solution}. $\ln\left(1+\sqrt{2}\right)$.
  ...
  \end{enumerate}
  \section{计算题(每小题 6 分，共 30 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 曲线$C$没有竖直渐近线和水平渐近线.
    ...
    \item[12.] \textbf{Solution}. 由积分中值定理，存在$\xi$介于$a$和$x$之间，使得$\int
    ...
    \item[14.] \textbf{Solution}. 方程变形为$\frac{\mathrm{d}x}{\mathrm{d}y}-\frac{2x}{y}
      =y^{3}$.
    ...
    \item[15.] \textbf{Solution}. 由题可知$f(1)=0$，$f'(x)=-\sin x^{2}$，所以
      \[
        \begin{aligned}
    ...
    \item[16.] \textbf{Solution}. $F'(x)=\mathrm{e}^{-x}\sin x$，在$\left(-\frac{\pi}{2}
  ...
  \section{综合题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] \textbf{Solution}. $f'(x)=\sqrt{1+x^{2}}-2x\sqrt{1+x^{2}}=(1-2x)\sqrt{1+x^{2}}$，
    ...
    \item[18.] \textbf{Solution}. 令$t-x=u$，则$\int_{-x}^{0}f(u)\mathrm{d}u=-\frac{x^{3}}{3}
      -x^{2}$.
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] \textbf{Proof}. 易知函数$\sqrt[n]{f(x)}$在闭区间$[0,1]$上连续，因而具有最小值$m$和最大值$M$.所以
      \[
    ...
    \item[20.] \textbf{Proof}. 由Taylor公式，将$f(x)$分别在$x=0$和$x=2$处展开得
      \[
        \begin{gathered}
