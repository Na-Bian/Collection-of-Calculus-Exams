...
\begin{document}
  ...
  \begin{enumerate}
    \item 当$x\to0$时，函数$f(x)=\frac{1}{x}\sin\frac{1}{x}$是【\quad】.
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 无界的但不是无穷大量 & \textbf{B.} 无穷大量 \\
        \textbf{C.} 有界的但不是无穷小量 & \textbf{D.} 无穷小量
      \end{tabular}
      \vspace{1em}
    ...
    \item 设$\{a_{n}\},\{b_{n}\},\{c_{n}\}$均为非负数列，且$\lim_{n\to\infty}a_{n}
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} 对任意正整数$n$，有$a_{n}<b_{n}$   & \textbf{B.} 对任意正整数$n$，有$b_{n}<c_{n}$   \\
        \textbf{C.} 数列$\{a_{n}\cdot c_{n}\}$发散 & \textbf{D.} 数列$\{b_{n}\cdot c_{n}\}$发散
      \end{tabular}
      \vspace{1em}
    ...
    \item 设函数$f(x)$满足$f'(x_{0})=f''(x_{0})=0$，且$f'''(x_{0})>0$，则【\quad】.
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $f'(x_{0})$是$f'(x)$的极大值 & \textbf{B.} $f(x_{0})$是$f(x)$的极大值            \\
        \textbf{C.} $f(x_{0})$是$f(x)$的极小值   & \textbf{D.} $(x_{0},f(x_{0}))$是曲线$y=f(x)$的拐点
      \end{tabular}
      \vspace{1em}
    ...
    \item 若$f'(2x)=\mathrm{e}^{-x}$，且$f(0)=1$，则$\int f(x)\mathrm{d}x=$【\quad】.
      ...
      \begin{tabular}{l@{\hspace{2cm}}l}
        \textbf{A.} $3x-\mathrm{e}^{-\frac{x}{2}}+C$  & \textbf{B.} $3x-2\mathrm{e}^{-\frac{x}{2}}+C$ \\
        \textbf{C.} $3x+4\mathrm{e}^{-\frac{x}{2}}+C$ & \textbf{D.} $3x-4\mathrm{e}^{-\frac{x}{2}}+C$
      \end{tabular}
      \vspace{1em}
    ...
    \item 设函数$f(x)$二阶可导，且$f''(x)<0$，$f(0)=0$，则【\quad】.
      ...
      \begin{tabular}{l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l}
        \textbf{A.} $f(2)<2f(1)$ & \textbf{B.} $f(2)>2f(1)$ & \textbf{C.} $2f(2)<f(1)$ & \textbf{D.} $2f(2)>f(1)$
      \end{tabular}
      \vspace{1em}
    ...
    \item 下列反常积分收敛的是【\quad】.
      ...
      \begin{tabular}{l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l@{\hspace{1.5cm}}l}
        \textbf{A.} $\int_{\mathrm{e}}^{+\infty}\frac{\ln x}{x}\mathrm{d}x$ & \textbf{B.} $\int_{\mathrm{e}}^{+\infty}\frac{1}{x\ln x}\mathrm{d}x$ & \textbf{C.} $\int_{\mathrm{e}}^{+\infty}\frac{1}{x\sqrt{\ln x}}\mathrm{d}x$ & \textbf{D.} $\int_{\mathrm{e}}^{+\infty}\frac{1}{x\ln^{2}x}\mathrm{d}x$
      \end{tabular}
      \vspace{1em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[7.] 设函数$y=f(x)$由参数方程$\begin{cases}
        x=t^{2}+1, \\
        y=4t-t^{2}
      \end{cases}(t\geq0)$确定，则$\lim_{n\to\infty}n\left[f\left(\frac{2n+1}{n}\right
      )-3\right]=\underline{\hspace{2cm}}$.

    \item[8.] 设函数$f(x)=\int_{0}^{3x}\mathrm{e}^{-t^2}\mathrm{d}t+2$，$g(x)$是$f
      (x)$的反函数，则$g'(2)=\underline{\hspace{3cm}}$.

    \item[9.] 设$f(x)=\frac{1}{1+x^{2}}+\sqrt{1-x^{2}}\int_{0}^{1} f(x)\mathrm{d}
      x$，则$\int_{0}^{1} f(x)\mathrm{d}x=\underline{\hspace{3cm}}$.

    \item[10.] 连续曲线段$y=\int_{0}^{x}\sqrt{\sin t}\mathrm{d}t(0\leq x\leq \pi)$的弧长$s
      =\underline{\hspace{3cm}}$.
  \end{enumerate}

  \section{计算题(每小题 7 分，共 42 分)}
  \begin{enumerate}
    \item[11.] 求曲线$y=\sqrt{\frac{x^{3}}{x-1}}(x>1)$的渐近线.
      \vspace{10em}

    \item[12.] 求极限$\lim_{n\to\infty}\left(\frac{1}{2n+\dfrac{1}{n}}+\frac{1}{2n+\dfrac{4}{n}}
      +\cdots+\frac{1}{2n+\dfrac{n^2}{n}}\right)$.
      \vspace{10em}

    \item[13.] 设方程$\mathrm{e}^{xy}+\frac{1}{\sqrt{3}}\int_{1}^{y}\sqrt{4-t^{2}}
      \mathrm{d}t=1$可以确定函数$y=y(x)$，求$\left.\frac{\mathrm{d}y}{dx}\right|_{x=0}$.
      \vspace{10em}

    \item[14.] 求方程$y''-3y'+2y=2\mathrm{e}^{x}$的积分曲线$y=y(x)$，使其在点$(0,
      1)$处的切线与曲线$y=x^{3}-3x+1$在该点处的切线重合.
      \vspace{12em}

    \item[15.] 设$f(x)=\int_{0}^{x}\mathrm{e}^{-t^2+2t}\mathrm{d}t$，求$I=\int_{0}
      ^{1}(x-1)^{2} f(x)\mathrm{d}x$.
      \vspace{12em}

    \item[16.] 设函数$f(x)=\int_{0}^{1}|t^{3}-x^{3}|\mathrm{d}t(0\leq x\leq 1)$，求$f
      (x)$的最值.
      \vspace{12em}
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[17.] 设$f(x)$连续，且满足$\lim_{x\to 0}\frac{f(x)-\tan x}{x}=1$，$F(x)=
      \int_{0}^{1}f(xt)\mathrm{d}t$. 求$F'(x)$，并讨论$F'(x)$的连续性.
      \vspace{12em}

    \item[18.] 设曲线$y=y(x)$是第一象限内的连续曲线，点$A(0,1),B(1,0)$分别为曲线与$y$轴及$x$轴的交点.
  ...
  \begin{enumerate}
    \item[19.] 设$f(x)$在$[0,2]$上连续，在$(0,2)$内可导，且$\lim_{x\to\frac{1}{2}}
      \frac{f(x)}{x-\frac{1}{2}}=0$，$f(2)=2\int_{1}^{\frac{3}{2}}f(x)\mathrm{d}x$.
      证明：存在$\xi\in(0,2)$，使得$f''(\xi)=0$.
      \vspace{13em}

    \item[20.] 设$f(x)$在$[0,1]$上有连续的二阶导数，且$|f''(x)|\leq1$，证明：
      \[
  ...
  \begin{enumerate}
    ...
    \item[8.] \textbf{Solution}. $\frac{1}{3}$.

      令$f(x)=2$得$x=0$，$f'(x)=3\mathrm{e}^{-9x^2}$，所以$g'(2)=\frac{1}{f'(g(2))}
      =\frac{1}{f'(0)}=\frac{1}{3}$.

    \item[9.] \textbf{Solution}. $\frac{\pi}{4-\pi}$.
  ...
  \begin{enumerate}
    \item[11.] \textbf{Solution}. 当$x\to 1^{+}$时，$y=\sqrt{\frac{x^{3}}{x-1}}\to
    ...
    \item[12.] \textbf{Solution}. 由定积分的定义，
      \[
        \begin{aligned}
    ...
    \item[13.] \textbf{Solution}. 方程$\mathrm{e}^{xy}+\frac{1}{\sqrt{3}}\int_{1}
    ...
    \item[14.] \textbf{Solution}. 齐次方程$y''-3y'+2y=0$的特征方程为$r^{2}-3r+2=0$，解得$r
      _{1}=1$，$r_{2}=2$.
    ...
    \item[15.] \textbf{Solution}. 由题可知$f(0)=0$，$f'(x)=\mathrm{e}^{-x^2+2x}$，所以
      \[
        \begin{aligned}
  ...
  \section{综合题(每小题 7 分，共 14 分)}
  \begin{enumerate}
    \item[17.] \textbf{Solution}. 当$x=0$时，$F(0)=0$.
    ...
    \item[18.] \textbf{Solution}. 由题可知点$C$的坐标为$(x,0)$，$S_{\text{梯形}OCMA}
  ...
  \section{证明题(每小题 5 分，共 10 分)}
  \begin{enumerate}
    \item[19.] \textbf{Proof}. 由$\lim_{x\to\frac{1}{2}}\frac{f(x)}{x-\frac{1}{2}}
    ...
    \item[20.] \textbf{Proof}. 由Taylor公式，$f(x)=f\left(\frac{1}{2}\right)+f'\left
