...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item[8.] \textbf{Solution}. $y=x$.
      ...
      $\lim_{x\to+\infty}\frac{\frac{1}{x}+\ln\left(1+\mathrm{e}^{x}\right)}{x}=\lim
      _{x\to+\infty}\left[\frac{1}{x^{2}}+\frac{\ln\left(1+\mathrm{e}^{x}\right)}{x}
      \right]=1$，

      $\lim_{x\to+\infty}\left[\frac{1}{x}+\ln\left(1+\mathrm{e}^{x}\right)-x\right
      ]=\lim_{x\to+\infty}\left[\ln\left(1+\mathrm{e}^{x}\right)-x\right]=0$.
  ...
  \begin{enumerate}
    ...
    \item[15.] 方程变形为$\frac{\mathrm{d}x}{\mathrm{d}y}-\frac{3x}{y}=-\frac{y}{2}$.
