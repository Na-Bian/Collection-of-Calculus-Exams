...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \item[13.] 求常数$a,b,c$的值，使得函数$f(x,y,z)=axy^{2}+byz+cx^{3}z^{2}$在点$M
  ...
  \begin{enumerate}
    ...
    \item \textbf{Solution}. 由题意可知$y_{2}-y_{1}=2\mathrm{e}^{x}$，$y_{3}-y_{2}
      ...
      因此通解为$(Ax+B)\mathrm{e}^{x}$，故原方程的通解为$y = (Ax+B)\mathrm{e}^{x}
      + x$，其中$A,B$为任意常数.
  ...
  \begin{enumerate}
    \item[11.] \textbf{Solution}. $\begin{cases}
      ...
      由链式法则，$\frac{\partial z}{\partial x}= \frac{\partial z}{\partial u}\cdot
      ...
      v}$，$\frac{\partial z}{\partial y}= \frac{\partial z}{\partial u}\cdot \frac{\partial
    ...
    \item[12.] \textbf{Solution}. 当$x\to0,y\to0$时，$\frac{x^{2}y^{2}}{\left(x^{2}+y^{2}\right)^{\frac{3}{2}}}
      ...
      = \lim_{x\to0}\frac{x^{4}}{(2x^{2})^{2}}= \frac{1}{4}$，所以$\lim_{\substack{x\to 0 \\ y\to 0}}
