...
\begin{document}
  \chapter{2022-2023学年微积分（B）（一）期中考试}
  ...
  \begin{enumerate}
    \item[11.] 设函数$f(u)$具有二阶连续导数，$z=f\left(\mathrm{e}^{x}\cos y\right
      )$满足
      \[
    ...
    \item[13.] 设曲面$\Sigma$为曲线$\begin{cases}
        3x^{2}+2y^{2}=12, \\
        z=0
    ...
    \item[15.] 设$P_{0}(x_{0},y_{0},z_{0})$为光滑曲面$S:\varphi(x,y,z)=0$外的一固定点，$P
      (x,y,z)$为$S$上任意一点. 证明：若$\left|\overrightarrow{P_0P}\right|$最短，则$\overrightarrow
      {P_0P}$必是曲面$S$在点$P$处的法向量.
      \vspace{11em}
  \end{enumerate}

  \chapter{2022-2023学年微积分（B）（一）期中考试参考答案}
  \section{基本计算题(每小题 6 分，共 60 分)}
  \begin{enumerate}
    ...
    \item \textbf{Solution}. 方程$\varphi\left(x^{2},\mathrm{e}^{y},z\right)=0$两边对$x$求导，得
      ...
      所以$\frac{\mathrm{d}u}{\mathrm{d}x}=f'_{1}+f'_{2}\cdot \frac{\mathrm{d}y}{\mathrm{d}x}
    ...
    \item \textbf{Solution}. 积分区域如图所示，其中$D_{1}=\{(x,y)|-1\leq y\leq 0,
      ...
      \begin{minipage}[c]{0.65\textwidth}
        由于$D_{1}$关于$y$轴对称，被积函数$y=x\sqrt{1-x^{2}+y^{2}}$关于$x$是奇函数，
  ...
  \section{综合题(每小题 8 分，共 40 分)}
  \begin{enumerate}
    \item[11.] \textbf{Solution}. $\frac{\partial z}{\partial x}= f'\left(\mathrm{e}
      ...
      所以$\frac{\partial^{2} z}{\partial x^{2}}+\frac{\partial^{2} z}{\partial
      y^{2}}= f''\left(\mathrm{e}^{x}\cos y\right)\cdot \mathrm{e}^{2x}= \left(4f
      \left(\mathrm{e}^{x}\cos y\right)+\mathrm{e}^{x}\cos y\right)\mathrm{e}^{2x}$，即$f
      ''(u) = 4f(u)+u$.

      齐次方程$f''-4f=0$的特征方程为$r^{2}-4=0$，解得$r=\pm 2$，所以齐次方程的通解为$C
      _{1}\mathrm{e}^{2x}+C_{2}\mathrm{e}^{-2x}$.
    ...
    \item[12.] \textbf{Solution}. 令$F(x,y,z) = xy+z$，$\nabla F = (y,x,1)$，
    ...
    \item[13.] \textbf{Solution}. 由题意可知曲面$\Sigma$的方程为$3(x^{2}+z^{2})+2
      y^{2}=12$，
    ...
    \item[14.] \textbf{Solution}. $\lim_{\substack{x\to 0 \\ y\to 0}}\frac{\sqrt{|xy|}\sin\left(x^{2}+y^{2}\right)}{x^{2}+y^{2}}
      ...
      = \lim_{x\to0}\sqrt{\frac{x^{2}}{2x^{2}}}= \frac{\sqrt{2}}{2}$，所以$\lim_{\substack{x\to 0 \\ y\to 0}}
      \frac{\sqrt{|xy|}\sin\left(x^{2}+y^{2}\right)}{\left(x^{2}+y^{2}\right)\sqrt{x^2+y^2}}
      \neq 0$，

      因此函数$f(x,y)$在原点处不可微.

    \item[15.] \textbf{Solution}. 考虑距离平方函数
      \[
