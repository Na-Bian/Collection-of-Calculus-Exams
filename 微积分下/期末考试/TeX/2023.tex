...
\begin{document}
  ...
  \begin{enumerate}
    \item 在空间直角坐标系中，方程 \( z = \sqrt{x^2 + y^2 - 1} \) 表示【\quad】.
    ...
    \item 设 \( z = f(x, y) \) 在点 \( (1,1) \) 处有 \( f_x(1,1) = f_y(1,1) = 1 \)，则必有【\quad】.
    ...
    \item 设函数 \( z = f(x, y) \) 的全微分为 \( \mathrm{d}z = 2x \mathrm{d}x + 3y \mathrm{d}y \)，则点 \( (0,0) \)【\quad】.
    ...
    \item 设 \( \Omega \) 是由锥面 \( z = \sqrt{x^2 + y^2} \) 和平行 \( z = 1 \) 所围成的空间区域，将 \( I = \displaystyle \iiint \limits_{\Omega} f(\sqrt{x^2 + y^2 + z}) \mathrm{d}x\,\mathrm{d}y\,\mathrm{d}z \) 化为柱坐标系下的累次积分，下列结果正确的是【\quad】.
    ...
    \item 设 \( S \) 是上半球面 \( x^2 + y^2 + z^2 = R^2 \) (\( z \geq 0, R > 0 \))，\( abc \neq 0 \)，则 \( \displaystyle\iint \limits_S (ax + by + cz) \mathrm{d}S = \)【\quad】.
