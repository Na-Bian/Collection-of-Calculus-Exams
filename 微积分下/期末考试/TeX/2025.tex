...
\begin{document}
  ...
  \begin{enumerate}
    \item 函数$f(x,y,z)=x^2+y^2-z^2$的等值面\textbf{不可能}是（\qquad）.
    ...
    \item 设$f(x,y)$在$(0,0)$处连续，且$\lim\limits_{\substack{x \to 0 \\ y \to 0}}\dfrac{f(x,y)-1}{x^2+y^2}=2$，则$f(x,y)$在点$(0,0)$处（\qquad）.
    ...
    \item 设幂级数$ \displaystyle\sum \limits_{n=0}^{\infty} a_n(x-1)^n $ 在 $ x = 3 $ 处条件收敛，则级数 $ \displaystyle\sum \limits_{n=1}^{\infty} \dfrac{a_n}{2^n}(x+1)^n$在$x=-3$处（\qquad）.
